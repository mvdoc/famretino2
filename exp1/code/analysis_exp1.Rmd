...
```{r, fig.height=5, fig.width=5}
pse_subj <- list()
pse_subj[[1]] <-  dpse_ses1 + matrix(rep(psep_ses1, nrow(dpse_ses1)), 
                                     byrow=T, nrow=nrow(dpse_ses1)) 
pse_subj[[2]] <-  dpse_ses2 + matrix(rep(psep_ses2, nrow(dpse_ses2)), 
                                     byrow=T, nrow=nrow(dpse_ses2)) 

plot_examplefit <- function(which_subj, ses=2, extreme_curves=NULL) {
  pse_subj_ses  <- pse_subj[[ses]]
  ...
  pse_subj_ses$pos_num <-
    factor(pse_subj_ses$pos_num, levels=seq(0, 7)*45)
  
  df_plot <- df %>%
    filter(subject == which_subj, session == as.character(ses)) 
  df_plot$pos_num <- 
  ...
  predict_subj %>%
    ...
    geom_segment(aes(x=pse, xend=pse, y=-10, yend=50), alpha=0.8, linetype='dashed', data=pse_subj_ses) +
    geom_line(size=0.8) +
    theme_Publication() +
...
for (ses in 1:2) {
  for (s in subjects) {
    plot <- plot_examplefit(s, ses, extreme_values[[s]])
    ggsave(paste('../img/example_fit_', s, '_ses', ses, '.png', sep=''),
           width=5, height=5)
  }
}
```
