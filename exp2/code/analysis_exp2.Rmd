...
for (ses in c('1', '2')) {
  ...
  df_s10_plot_ab <-
    df %>%
    filter(subject == 's10', session == ses) %>%
  ...
  pse_s10[[ses]] <- pse_s10_plot
  df_s10[[ses]] <- df_s10_plot
}
```
```{r, fig.height=6, fig.width=8}
for (ses in c('1', '2')) {
predict_subj %>%
  ...
  geom_segment(aes(x=tpse, xend=tpse, y=-10, yend=50), alpha=0.8, linetype='dashed', data=pse_s10[[ses]]) +
  geom_line() +
  ...
  stat_summary(aes(morph, response_bin), fun.y=function(x) sum(x)/length(x)*100, geom='point', 
               data=df_s10[[ses]], size=0.9, show.legend=F)
ggsave(paste('../img/examplefit_s10', '_ses', ses, '.png', sep=''), width=8, height=6)
}
```
...
Compute correlations across scales to check how consistent they are

```{r}
tidy(round(cor(df_quest[c('ios', 'wescale', 'sci1', 'sci2')]), 2))
min(cor(df_quest[c('ios', 'wescale', 'sci1', 'sci2')]))
```
...
# compute log-likelihood ratio manually
m1_loglik <- logLik(m1)
m2_loglik <- logLik(m2)
lr <- as.numeric(2 * (m2_loglik - m1_loglik))
plr <- pchisq(lr, 1, lower.tail=F)

print(paste("Log-likelihood ratio:", lr, "pvalue:", plr))
```
```{r}
