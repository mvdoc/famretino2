function cssmodel_vtcdata_facetask(subject, roi, debug)
% CSSMODEL_VTCDATA(subject, roi, debug)
%
% This function is a modified version of the cssmodel_example.m script
% available from http://kendrickkay.net/socmodel to be run on the VTCdata for
% the face task
...
fnout = [outdir, '/', sprintf('sub-%02d_%s_facetask', subject, roilabels{roi})];
if debug
    fnout = [fnout '_dbg'];
end
...
stimulus = reshape(stimulus,100*100,25)';

%% Select only the stimuli corresponding to the face task
% From http://kendrickkay.net/vtcdata/ :
% "For the task experiment, there are 75 beta weights ordered in three groups 
% of 5*5=25, where the first group consists of the medium faces under the digit 
% task, the second group consists of the medium faces under the dot task, and 
% the third group consists of the medium faces under the face task. 
...
% In this version we FIX the n parameter to 0.2 since we don't have enough data
seed = [(1+res)/2 (1+res)/2 res 1 0.2];
...
data = betamn(:, 2);  % take second experiment only: task-based
% select the ROI
data = data{roi};
...
for iface = 1:1
   randidx = randperm(25) + (iface-1)*25;
   for ig = 1:10
       idx = randidx(groups{ig});
...
assert(isequal(unique(sum(resampling == -1, 2)), [2, 3]'))
